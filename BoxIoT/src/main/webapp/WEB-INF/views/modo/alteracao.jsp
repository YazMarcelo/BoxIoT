...
<mt:simpletemplate title="Alteração de Modo">
        ...
        <div class="row">
            <div class="col-md-12">
                <div class="card">
                    ...
                    <div class="card-body">
                    	<form action="#">
                            <input type="hidden" id="id_controle" name="id_controle" value="${modo.id}" />
                            ...
                            <div class="row">
                                <div class="col-md-12">
                                    <div class="form-group">
                                         ...
                                         <input id="descricao" name="descricao" class="form-control" required="required" type="text" value="${modo.descricao}">
                                    </div>
                                </div>
                            </div>
                                         ...
                                         <div class="card-collapse">
											...
											<h5 style="margin-bottom: 5px;margin-top: 10px;"><b>Itens</b></h5>
												<c:forEach items="${itens}" var="item"><c:if test = "${item.idLocal == local.id}">
													...
													<c:forEach items="${itensModo}" var="itemModo">
													<c:if test = "${item.id == itemModo.idItem}">
														<c:set var = "theString" value = "ok"/>
														<c:set var = "porcentagem" value = "${itemModo.porcentagem}"/>
													</c:if>
													</c:forEach>
														<c:choose>
    													<c:when test="${fn:contains(theString, 'ok')}">
                     										<div class="card-body div-item">
                                      						...
                                      						<div class="form-check">
                                          						<label class="form-check-label">
                                              						<input class="form-check-input on" type="radio" name="radio-${item.id}" value="true" checked>
                                              							<label>Ligado <b class="porcentagem">${porcentagem}</b>%</label>
                                              						<span class="circle">
                                                  						<span class="check"></span>
                                              						</span>
                                          						</label>
                                          						<br>
										  						<div class="slider-primary" data-plugin-slider data-plugin-options='{ "value": ${porcentagem}, "range": "min", "max": 100 }' data-plugin-slider-output="#slider-${item.id}">
										  							<input id="slider-${item.id}" name="porcentagem" class="slider-item" type="hidden" value="${porcentagem}" />
									      						</div>
                                      						...
                                      						<div class="form-check">
                                          						<label class="form-check-label">
                                              						<input class="form-check-input off" type="radio" name="radio-${item.id}" value="false">
                                              							Desligado
                                              								<span class="circle">
                                                  								<span class="check"></span>
                                              								</span>
         											...
         											<div class="card-body div-item">
                    									...
                    									<div class="col-sm-5 checkbox-radios">
                                      						<div class="form-check">
                                          						...
                                          						<br>
										  						<div style="display:none;" class="slider-primary" data-plugin-slider data-plugin-options='{ "value": 100, "range": "min", "max": 100 }' data-plugin-slider-output="#slider-${item.id}">
